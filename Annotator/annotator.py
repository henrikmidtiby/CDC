...
class RasterAnnotator:
    def __init__(self, raster_file, alphachannel=True):
        ...
        self.check_buttons = CheckButtons(
            self.check_ax, [f"Band {i+1}" for i in range(self.bands)], self.selected_bands
        )
    ...
    def _annotate(self, x, y):
        ...
        valid_mask = (
            (x + r >= 0)
            & (x + r < self.annotated_mask.shape[1])
            & (y + c >= 0)
            & (y + c < self.annotated_mask.shape[0])
        )
    ...
    def save_mask(self, event):
        ...
        if output_file and self.annotated_mask is not None:
...
parser.add_argument("reference_image", help="Path to the .tiff file that you want to Annotate.")
# parser.add_argument(--)
